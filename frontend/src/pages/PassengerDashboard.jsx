    ...
    <div className="passenger-app">
      ...
      {rideState === 'IDLE' && (
        <div className="fixed-address-bar animate-fade-in">
          ...
          {/* === PREÇO IMBATÍVEL ZOMP (Visível na Tela Inicial) === */}
          <div style={{
            marginTop: '14px',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
              ...
              <div>
                <h4 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 900, color: '#b91c1c' }}>
                  PREÇO IMBATÍVEL ZOMP
                </h4>
                <p style={{ margin: 0, fontSize: '0.72rem', fontWeight: 600, color: '#991b1b' }}>
                  Viu mais barato na Uber ou 99? Envie o print e pague até R$ 3,00 a menos!
                </p>
              </div>
            </div>
                ...
                onChange={async (e) => {
                  ...
                  reader.onload = async (ev) => {
                    ...
                    try {
                      ...
                      // Se os endereços estiverem preenchidos, passa para PRICED
                      if (destAddr.trim().length >= 2) {
                        handleForceCalculate();
                      } else {
                        alert(`✅ Print Uber/99 de R$ ${printVal.toFixed(2)} validado com sucesso!\nDesconto Zomp: - R$ ${result.discountAmount.toFixed(2)}.\n\nAgora digite o seu destino para chamar o carro!`);
                      }
                    } catch (err) {
                      setIsAnalyzingScreenshot(false);
                      ...
                      setHasCompetitionDiscount(false);
                      setManualPriceInput('');
                      alert(err.message || 'Erro ao validar print da concorrência.');
                    }
                  };
                  reader.readAsDataURL(file);
                }}
              />
              <label htmlFor="imbativel-screenshot-idle" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', margin: 0 }}>
                {isAnalyzingScreenshot ? (
                  <span style={{ fontSize: '0.82rem', fontWeight: 800, color: '#b91c1c' }}>
                    🔍 Analisando print da Uber/99...
                  </span>
                ) : (
                  <>
                    <Camera size={20} color="#ef4444" />
                    <span style={{ fontSize: '0.82rem', fontWeight: 800, color: '#b91c1c' }}>
                      📸 Toque aqui para Enviar Print da Uber ou 99
                    </span>
                  </>
                )}
              </label>
